...
def pick_resolution(graph, target_avg_ani, steps=101, seed=1953):
    ...
    for res in rich.progress.track(
        np.linspace(1, 0, steps), description="Step", total=steps, transient=True
    ):
        random.seed(seed)
...
def cluster(
    fasta: Path = FASTA,
    outfile: Path = typer.Argument(..., file_okay=True, dir_okay=False),
    ...
    easy_cluster: bool = typer.Option(
        ...
        help="Use easy-cluster or easy-linclust command. Ignored if --preset nt-cluster is used.",
    ),
    threads: int = Threads,
    ...
    1. Steinegger, M. and Soeding, J., 2017.
    ...
    base_command = "easy-cluster"
    if preset == PRESET.NT_CLUSTER:
        base_command = "easy-search"
    elif not easy_cluster:
        base_command = "easy-linclust"
    ...
    if preset == PRESET.NT_CLUSTER:
        ...
        fasta = tmp
        arg2 = str(fasta)
        arg3 = str(outfile.stem + ".AvA")
        colnames = "query,target,pident,qlen,tlen,qstart,qend,tstart,tend,alnlen,evalue,bits,qcov,tcov"
    else:
        arg2 = str(outfile)
        arg3 = ""
    ...
    command = (
        ...
        f"{'' if sensitivity is None or not easy_cluster else f'-s {sensitivity} '}"
        ...
        f"{'--format-output ' + colnames + ' ' if preset == PRESET.NT_CLUSTER else ''}"
        # general options
        f"--threads {threads} "
    ...
    if preset == PRESET.NT_CLUSTER:
        logging.info("Performing ANI postprocessing...")
        df = pd.read_csv(arg3, sep="\t", names=colnames.split(","))
        ...
        for row in rich.progress.track(
            df.itertuples(), total=len(df), transient=True, description="Pairs"
        ):
            nodes.extend([row.query, row.target])
            if (row.qcov >= min_cov) or (row.tcov >= min_cov):
        ...
        leiden_resolution = pick_resolution(graph, 0.8)
